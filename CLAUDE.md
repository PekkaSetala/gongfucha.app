...
- **Schedule adjustment**: changing leaf amount from recommended scales all steep times proportionally, clamped to `0.5×` on the low end and each preset's `maxAdjust` (0.6–3.0) on the high end. A tapered curve applies — early steeps get the full multiplier, later steeps converge toward 1.0. See `src/lib/brewing.ts:40-64` and `src/data/teas.ts` for per-tea ceilings.
